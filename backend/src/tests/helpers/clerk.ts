...
export interface TestClerkUser { clerkId: string; jwt: string; devBrowserJwt: string; }
...
export async function createTestClerkUser(emailPrefix: string): Promise<TestClerkUser> {
  ...
  const tokenRes = await fetch(`${CLERK_API}/sessions/${sessionId}/tokens`, { method: "POST", headers: authHeader() });
  if (!tokenRes.ok) throw new Error(`session token failed (${tokenRes.status}): ${await tokenRes.text()}`);
  const { jwt } = await tokenRes.json() as { jwt: string };
  if (!jwt) throw new Error(`No JWT returned`);

  return { clerkId, jwt, devBrowserJwt };
}

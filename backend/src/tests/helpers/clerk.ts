...
export async function createTestClerkUser(emailPrefix: string): Promise<TestClerkUser> {
  ...
  if (!userRes.ok) throw new Error(`Clerk createUser failed: ${await userRes.text()}`);
  ...
  if (!devBrowserRes.ok) throw new Error(`Dev browser init failed: ${await devBrowserRes.text()}`);
  ...
  if (!signInTokenRes.ok) throw new Error(`Clerk createSignInToken failed: ${await signInTokenRes.text()}`);
  ...
  if (!exchangeRes.ok) throw new Error(`FAPI ticket exchange failed: ${await exchangeRes.text()}`);
  ...
  if (!sessionId) throw new Error(`No session ID in exchange response: ${JSON.stringify(exchangeData)}`);
  ...
  if (!tokenRes.ok) throw new Error(`Session token fetch failed (${tokenRes.status}): ${await tokenRes.text()}`);

  const tokenData = (await tokenRes.json()) as { jwt?: string };
  if (!tokenData.jwt) throw new Error(`No JWT in token response: ${JSON.stringify(tokenData)}`);

  return { clerkId, jwt: tokenData.jwt };
}
